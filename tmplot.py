...
def arg_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Plotter that supports file and pipe input for quick description",
    ...
    parser.add_argument(
        ...
        type=str,
        default="float",
        help="Input x data type specification [default: float]",
    ...
    parser.add_argument(
        ...
        type=str,
        default="float",
        help="Input y data type specification [default: float]",
        choices=["float", "int", "str"],
    )
    ...
    parser.add_argument("-l", "--label", type=str, help="Main plotting label [default: <None>]")
    parser.add_argument(
        "--xlim",
        type=str,
    ...
    parser.add_argument(
        "--xnorm",
        action="store_true",
        help="Flag whether inputed x data normalization" "If the data type is str, an error will occur",
    )
    parser.add_argument(
        "--ynorm",
        action="store_true",
        help="Flag whether inputed y data normalization" "If the data type is str, an error will occur",
    )
    parser.add_argument(
        "--xstd",
        action="store_true",
        help="Flag whether inputed x data standardization" "If the data type is str, an error will occur",
    )
    parser.add_argument(
        "--ystd",
        action="store_true",
        help="Flag whether inputed y data standardization" "If the data type is str, an error will occur",
    )
    ...
    parser.add_argument(
        "--jpg",
        action="store_true",
        help="Flag whether JPG output is performed. [default: <PREFIX>.png]",
    )
    parser.add_argument(
        "--pdf",
        action="store_true",
        help="Flag whether PDF output is performed. [default: <PREFIX>.png]",
    )
    parser.add_argument(
        "--ps",
        action="store_true",
        help="Flag whether PS output is performed. [default: <PREFIX>.png]",
    )
    parser.add_argument(
        "--transparent",
        action="store_true",
    ...
    # bar mode option
    parser.add_argument("--bar-width", type=float, default=0.8, help="Value of bar width [default: 0.8]")
    ...
    # joint mode option
    parser.add_argument("--joint-ratio", type=int, default=4, help="Ratio of jointplot [default: 4]")
    parser.add_argument("--joint-marker", type=str, default="+", help="Marker of jointplot [deafult: +]")
...
def common_plotter(args):
    ...
    if args.mode == "plot" or args.mode == "scatter" or args.mode == "window" or args.mode == "joint":
        ...
        if args.hist_bins == 0 and args.hist_bins_width == 0:
            # Sturges' rule.
            print(
                "[INFO] number of histogram bins is determined by Sturges's rurle",
                file=sys.stdout,
            )
            ...
            if args.hist_bins != 0 and args.hist_bins_width != 0:
                print(
                    "[WARN] When --hist-bins-width and --hist-bins are used together, --hist-bins takes precedence",
                    file=stdout,
                )
        ...
        if args.ydata != "-":
            print(
                "[WARN] If violin or box mode, inputed ydata is ignored",
                file=sys.stdout,
            )
        if args.xdata == "-":
            xdata = data_from_pipe1(args.xtype)
    ...
    if args.mode == "plot":
        plots = ax.plot(
            ...
            color=args.color,
            label=args.label,
            linewidth=args.plot_line_width,
            linestyle=args.plot_line_style,
        )
        if args.label != None:
        ...
        if args.xlog == True:
            ...
            n, bins, patches = ax.hist(
                xdata,
                bins=np.logspace(np.min(xdata), np.max(xdata), bins),
                color=args.color,
                label=args.label,
            )
        if args.label != None:
        ...
        if args.hist_peak_highlight == True:
            ...
            hist_peak_list = np.linspace(
                np.min(bins) + (bins[1] - bins[0]) / 2,
                np.max(bins) - (bins[1] - bins[0]) / 2,
                len(bins) - 1,
            )
            ...
            lines = plt.vlines(
                ...
                0,
                n[np.argmax(n)] * 1.1,
                color=args.hist_peak_highlight_color,
                label="peak : x = {}".format(hist_peak),
            )
            print(
                "[INFO] Color of histogram peak highlight bar is {}".format(args.hist_peak_highlight_color),
                file=sys.stdout,
            )
        ...
        if args.hist_cumulative == True:
            ...
            lines = ax2.plot(
                ...
                y2,
                ls="--",
                color=args.hist_cumulative_color,
                label="culative ratio",
            )
            print(
                "[INFO] Color of histogram cumulative ratio plot is {}".format(args.hist_cumulative_color),
                file=sys.stdout,
            )
        ...
        if args.bar_width > 1.0:
            print(
                "[WARN] If bar width is larger than 1.0, bar graphs may be difficult to see.",
                file=sys.stdout,
            )
        if args.label != None:
        ...
        if args.barh_height > 1.0:
            print(
                "[WARN] If barh height is larger than 1.0, bar graphs may be difficult to see.",
                file=sys.stdout,
            )
        if args.label != None:
        ...
        window = ax.plot(
            ...
            color=args.color,
            label=args.label,
            linewidth=args.window_line_width,
            linestyle=args.window_line_style,
        )
        if args.label != None:
        ...
        jointplot = sns.jointplot(
            x=xdata,
            y=ydata,
            ratio=args.joint_ratio,
            marginal_ticks=True,
            marker=args.joint_marker,
        )
    ...
    if args.xline != None:
        ...
        for i in range(0, len(xlines_list)):
            xline = ax.axvline(
                xlines_list[i],
                color=args.xline_color,
                label="x = {}".format(xlines_list[i]),
            )
            print(
                "[INFO] plot additional xline at {}".format(xlines_list[i]),
                file=sys.stdout,
            )
            legend_list.append(xline)
        print(
            "[INFO] color of addtional xline is {}".format(args.xline_color),
            file=sys.stdout,
        )
    if args.yline != None:
        ...
        for i in range(0, len(ylines_list)):
            yline = ax.axhline(
                ylines_list[i],
                color=args.yline_color,
                label="y = {}".format(ylines_list[i]),
            )
            print(
                "[INFO] plot additional yline at {}".format(ylines_list[i]),
                file=sys.stdout,
            )
            legend_list.append(yline)
        print(
            "[INFO] color of addtional yline is {}".format(args.yline_color),
            file=sys.stdout,
        )
...
def figsize_parser(figsize):
    ...
    if len(figsize_split) != 2:
        print(
            "[ERROR] plotting figsize parser error, not include 2 number",
            file=sys.stderr,
        )
        print('[ERROR] For --figsize, use "[6.4x4.8]" as example', file=sys.stderr)
        sys.exit(1)
...
def lines_parser(lines):
    ...
    if "[" not in lines:
        ...
        print(
            '[ERROR] For --xline or --yline, use "[-1.2]" or "[10,100, ...]" as example',
            file=sys.stderr,
        )
        sys.exit(1)
...
def range_parser(lim_range):
    ...
    if len(lim_range_list) != 2:
        print(
            "[ERROR] plotting range parser error, seems to be not include number",
            file=sys.stderr,
        )
        print('[ERROR] For --xlim or --ylim, use "[10:100]" as example', file=sys.stderr)
        sys.exit(1)
